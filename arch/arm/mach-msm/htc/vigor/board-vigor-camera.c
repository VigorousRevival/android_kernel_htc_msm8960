...
static struct regulator *vigor_reg_8058_l14 = NULL;
static struct regulator *vigor_reg_8058_l15 = NULL;
static struct regulator *votg_2_8v_switch = NULL;

#ifdef CONFIG_RAWCHIP
static int vigor_use_ext_1v2(void)
{
	return 0;
}
static int config_rawchip_on_gpios(void);

static int vigor_rawchip_vreg_on(void)
{
	int rc;
	pr_info("[CAM] %s\n", __func__);

	
	rc = camera_sensor_power_enable("8058_l14", 2850000, &vigor_reg_8058_l14);
	...
	rc = gpio_request(VIGOR_GPIO_V_CAM_D1V2_EN, "CAM_D1V2");
	if (rc < 0) {
		pr_err("[CAM] sensor_power_enable(\"gpio %d\", 1.2V) FAILED %d\n", VIGOR_GPIO_V_CAM_D1V2_EN, rc);
		goto enable_1v2_fail;
	}
	gpio_direction_output(VIGOR_GPIO_V_CAM_D1V2_EN, 1);
	gpio_free(VIGOR_GPIO_V_CAM_D1V2_EN);
	...
	rc = gpio_request(VIGOR_GPIO_MCLK_SWITCH, "CAM_SEL");
	if (rc < 0)
	{
		pr_err("[CAM] GPIO (%d) request fail\n", VIGOR_GPIO_MCLK_SWITCH);
		goto lcmio_hi_fail;
	}
	gpio_direction_output(VIGOR_GPIO_MCLK_SWITCH, 0);
	gpio_free(VIGOR_GPIO_MCLK_SWITCH);

	return rc;

lcmio_hi_fail:
	camera_sensor_power_disable(vigor_reg_8058_l9);
enable_analog_fail:
	gpio_request(VIGOR_GPIO_V_CAM_D1V2_EN, "CAM_D1V2_EN");
	gpio_direction_output(VIGOR_GPIO_V_CAM_D1V2_EN, 0);
	gpio_free(VIGOR_GPIO_V_CAM_D1V2_EN);
enable_1v2_fail:
	camera_sensor_power_disable(vigor_reg_8058_l8);
enable_1v8_fail:
	camera_sensor_power_disable(vigor_reg_8058_l14);
enable_VCM_fail:
	return rc;
}
static void config_rawchip_off_gpios(void);
static int vigor_rawchip_vreg_off(void)
{
	int rc = 0;
	...
	rc = gpio_request(VIGOR_GPIO_V_CAM_D1V2_EN, "RAW_1V2_EN");
	if (rc < 0) {
		pr_err("[CAM] sensor_power_enable(\"gpio %d\", 1.2V) FAILED %d\n", VIGOR_GPIO_V_CAM_D1V2_EN, rc);
		goto ville_rawchip_vreg_off_fail;
	}
	...
	rc = camera_sensor_power_disable(vigor_reg_8058_l14);
...
static uint32_t rawchip_on_gpio_table[] = {
	GPIO_CFG(VIGOR_GPIO_RAW_RSTN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 
	GPIO_CFG(VIGOR_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 
	GPIO_CFG(VIGOR_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_DO, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_DI, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_CS, 1 , GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_CLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GPIO_CAM_MCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
...
static uint32_t rawchip_off_gpio_table[] = {
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_DO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_DI, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_CS, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GPIO_MCAM_SPI_CLK, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_CAM_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_CAM_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_CAM_ID, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(VIGOR_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), 
	GPIO_CFG(VIGOR_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), 
	GPIO_CFG(VIGOR_GPIO_CAM_MCLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_16MA), 
};
...
static struct msm_camera_rawchip_info msm_rawchip_board_info = {
	.rawchip_reset	= VIGOR_GPIO_RAW_RSTN,
	.rawchip_intr0	= MSM_GPIO_TO_INT(VIGOR_GPIO_RAW_INTR0),
	.rawchip_intr1	= MSM_GPIO_TO_INT(VIGOR_GPIO_RAW_INTR1),
	.rawchip_spi_freq = 27, 
	.rawchip_mclk_freq = 24, 
	.camera_rawchip_power_on = vigor_rawchip_vreg_on,
	.camera_rawchip_power_off = vigor_rawchip_vreg_off,
	.rawchip_use_ext_1v2 = vigor_use_ext_1v2,
};
...
static uint16_t msm_cam_gpio_tbl[] = {
	VIGOR_GPIO_CAM_MCLK, 
	VIGOR_GPIO_RAW_INTR0,
	VIGOR_GPIO_RAW_INTR1,
	VIGOR_GPIO_MCAM_SPI_CLK,
	VIGOR_GPIO_MCAM_SPI_CS,
	VIGOR_GPIO_MCAM_SPI_DI,
	VIGOR_GPIO_MCAM_SPI_DO,
};
...
static struct msm_camera_sensor_platform_info sensor_s5k3h2yx_board_info = {
	...
	.sensor_reset	= 0,
	.sensor_pwd	= VIGOR_GPIO_CAM_PWDN,
	.vcm_pwd	= VIGOR_GPIO_CAM_VCM_PD,
	.vcm_enable	= 1,
};
...
static uint32_t camera_on_gpio_table[] = {
	GPIO_CFG(VIGOR_CAM_I2C_SDA, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_CAM_I2C_SCL, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_CAM_ID, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
	GPIO_CFG(VIGOR_GPIO_CAM_MCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA),
};
...
static int Villec2_mt9v113_vreg_on(void)
{
	...
	rc = gpio_request(VIGOR_GPIO_CAM2_RSTz, "mt9v113");
	if (rc < 0) {
		pr_err("[CAM] %s:VIGOR_GPIO_CAM2_RSTz gpio %d request failed, rc=%d\n", __func__,  VIGOR_GPIO_CAM2_RSTz, rc);
		goto init_fail;
	}
	gpio_direction_output(VIGOR_GPIO_CAM2_RSTz, 1);
	msleep(2);
	gpio_free(VIGOR_GPIO_CAM2_RSTz);
	...
	gpio_direction_output(VIGOR_GPIO_MCLK_SWITCH, 1);
	gpio_free(VIGOR_GPIO_MCLK_SWITCH);
...
static int Villec2_mt9v113_vreg_off(void)
{
	...
	gpio_direction_output(VIGOR_GPIO_CAM2_RSTz, 0);
	msleep(2);
	gpio_free(VIGOR_GPIO_CAM2_RSTz);
...
static struct msm_camera_sensor_platform_info sensor_mt9v113_board_info = {
	...
	.sensor_reset	= VIGOR_GPIO_CAM2_RSTz,
	.sensor_pwd = VIGOR_GPIO_CAM2_PWDN,
	.vcm_pwd	= 0,
	.vcm_enable	= 1,
};
