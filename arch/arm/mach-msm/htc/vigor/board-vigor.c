...
static uint32_t mhl_reset_pin_ouput_table[] = {
	GPIO_CFG(VIGOR_GPIO_MHL_RESET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};

static uint32_t mhl_usb_switch_ouput_table[] = {
	GPIO_CFG(VIGOR_GPIO_MHL_USB_SWITCH, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
...
void config_vigor_usb_id_gpios(bool output)
{
	if (output) {
...
static void vigor_usb_dpdn_switch(int path)
{
	...
	case PATH_MHL:
	{
		...
		gpio_set_value(VIGOR_GPIO_MHL_USB_SWITCH, (mhl ^ !polarity) ? 1 : 0);
		break;
	}
	}
...
static int64_t vigor_get_usbid_adc(void)
{
	int mean_adc;
	int adc[5];
	int i, ret = 0;
...
static uint32_t mhl_gpio_table[] = {
	GPIO_CFG(VIGOR_GPIO_MHL_RESET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(VIGOR_GPIO_MHL_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
};
...
static T_MHL_PLATFORM_DATA mhl_sii9234_device_data = {
	.gpio_intr = VIGOR_GPIO_MHL_INT,
	.gpio_reset = VIGOR_GPIO_MHL_RESET,
	.ci2ca = 0,
...
static uint32_t gsbi4_gpio_table[] = {
	GPIO_CFG(VIGOR_CAM_I2C_SDA, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
...
static uint32_t gsbi5_gpio_table[] = {
	GPIO_CFG(VIGOR_TP_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
...
static uint32_t gsbi7_gpio_table[] = {
	GPIO_CFG(VIGOR_GENERAL_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(VIGOR_GENERAL_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi7_gpio_table_gpio[] = {
	GPIO_CFG(VIGOR_GENERAL_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_GENERAL_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi10_gpio_table[] = {
	GPIO_CFG(VIGOR_SENSOR_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(VIGOR_SENSOR_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi10_gpio_table_gpio[] = {
	GPIO_CFG(VIGOR_SENSOR_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VIGOR_SENSOR_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static int vigor_ts_cy8c_set_rst(int on)
{
	...
	struct pm8058_gpio_cfg tp_rst[] = {
		{ /* TW RST Set LOW */
			PM8058_GPIO_PM_TO_SYS(VIGOR_TP_RST),
			{
...
static int vigor_ts_cy8c_power(int on)
{
	printk(KERN_INFO "%s():\n", __func__);
	if (on)
		vigor_ts_cy8c_set_rst(1);

	return 0;
}

static int vigor_ts_cy8c_reset(void)
{
	printk(KERN_INFO "[TP] HW reset touch.\n");
	vigor_ts_cy8c_set_rst(0);
	msleep(10);
	vigor_ts_cy8c_set_rst(1);
	msleep(200);
...
struct cy8c_i2c_platform_data vigor_ts_cy8c_data[] = {
	{
		.version = 0x0C,
		.timeout = 1,
		.unlock_attr = 1,
		...
		.abs_width_min = 0,
		.abs_width_max = 512,
		.power = vigor_ts_cy8c_power,
		.gpio_irq = VIGOR_TP_ATT_N_XB,
		/*.filter_level = {40, 80, 942, 982},*/
		.reset = vigor_ts_cy8c_reset,
	},
...
static void vigor_ts_cy8c_set_system_rev(uint8_t rev)
{
	ssize_t i = 0;

	if (rev >= PVT_VERSION)
		for (i = 0; i < sizeof(vigor_ts_cy8c_data)/sizeof(struct cy8c_i2c_platform_data); i++)
			vigor_ts_cy8c_data[i].auto_reset = 1;
}

static struct i2c_board_info msm_i2c_gsbi5_info[] = {
	{
		I2C_BOARD_INFO(CYPRESS_TMA_NAME, 0x67),
		.platform_data = &vigor_ts_cy8c_data,
		.irq = MSM_GPIO_TO_INT(VIGOR_TP_ATT_N_XB)
	},
};
...
static struct platform_device rpm_regulator_early_device __devinitdata = {
	...
	.id	= 0,
	.dev	= {
		.platform_data = &vigor_rpm_regulator_early_pdata,
	},
};
...
static struct platform_device rpm_regulator_device __devinitdata = {
	...
	.id	= 1,
	.dev	= {
		.platform_data = &vigor_rpm_regulator_pdata,
	},
};
...
static void __init vigor_early_memory(void)
{
...
static struct tpa2051d3_platform_data tpa2051d3_pdata = {
	.gpio_tpa2051_spk_en = VIGOR_AUD_HP_EN,
...
static struct i2c_board_info __initdata mpu3050_GSBI10_boardinfo[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(VIGOR_GYRO_INT),
...
#endif
#ifdef CONFIG_SERIAL_MSM_HS_BRCM
	msm_uart_dm1_pdata.wakeup_irq = gpio_to_irq(VIGOR_GPIO_BT_HOST_WAKE);
...
static void __init vigor_init(void)
{
	...
	if (system_rev > 2 || speed_bin == 0x1) {
		htc_headset_pmic_data.key_gpio =
			PM8058_GPIO_PM_TO_SYS(VIGOR_AUD_REMO_PRES);
	...
	platform_add_devices(vigor_devices,
			     ARRAY_SIZE(vigor_devices));
	...
	if (properties_kobj) {
          rc = sysfs_create_group(properties_kobj, &vigor_properties_attr_group);
	}
...
#endif
	vigor_ts_cy8c_set_system_rev(system_rev);
...
static void __init vigor_fixup(struct tag *tags,
...
MACHINE_START(VIGOR, "vigor")	
	...
	.init_very_early = vigor_early_memory,
	.restart = msm_restart,
MACHINE_END
