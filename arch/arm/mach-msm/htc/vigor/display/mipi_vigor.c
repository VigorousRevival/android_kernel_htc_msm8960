...
static int mipi_vigor_send_cmd(struct dsi_cmd_desc *cmd, unsigned int len, bool clk_ctrl)
{
	int ret = 0;
...
static struct msm_panel_common_pdata *mipi_vigor_pdata;

static int mipi_vigor_lcd_init(void);
...
void mipi_vigor_panel_type_detect(void)
{
...
static int mipi_vigor_lcd_on(struct platform_device *pdev)
{
	...
	mipi  = &mfd->panel_info.mipi;
        ...
        mipi_vigor_send_cmd(mipi_power_on_cmd, mipi_power_on_cmd_size, false);
...
static int mipi_vigor_display_on(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;
  unsigned int val = 0;
  ...
  mipi_vigor_send_cmd(novatek_display_on_cmds, ARRAY_SIZE(novatek_display_on_cmds), true);
...
static int mipi_vigor_lcd_off(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;
...
static int mipi_vigor_display_off(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;

  mfd = platform_get_drvdata(pdev);

  mipi_vigor_send_cmd(mipi_power_off_cmd, mipi_power_off_cmd_size, true);
...
inline void mipi_dsi_set_backlight(struct msm_fb_data_type *mfd, int level)
{
	...
	if (mfd->bl_level == 0 ||
        ...
        mipi_vigor_send_cmd(novatek_cmd_backlight_cmds, ARRAY_SIZE(novatek_cmd_backlight_cmds), true);

	bl_level_prevset = mfd->bl_level;
}

static void mipi_vigor_set_backlight(struct msm_fb_data_type *mfd)
{
  
        mipi_dsi_set_backlight(mfd, mfd->bl_level);
}

static int mipi_vigor_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_vigor_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static struct platform_driver this_driver = {
	.probe  = mipi_vigor_lcd_probe,
	.driver = {
		.name   = "mipi_vigor",
	},
};

static struct msm_fb_panel_data vigor_panel_data = {
	.on		= mipi_vigor_lcd_on,
        .off		= mipi_vigor_lcd_off,
        .set_backlight  = mipi_vigor_set_backlight,
	.late_init = mipi_vigor_display_on,
	.early_off = mipi_vigor_display_off,
};

static int ch_used[3];

int mipi_vigor_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	ret = mipi_vigor_lcd_init();
	if (ret) {
		pr_err("mipi_vigor_lcd_init() failed with ret %u\n", ret);
		return ret;
	}

	pdev = platform_device_alloc("mipi_vigor", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	vigor_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &vigor_panel_data,
		sizeof(vigor_panel_data));
	if (ret) {
		printk(KERN_ERR
